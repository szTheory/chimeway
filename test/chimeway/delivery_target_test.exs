defmodule Chimeway.DeliveryTargetTest do
  ...
  test "lifecycle operations only authorize their documented source states" do
    ...
    for lifecycle <- [
          &DeliveryTargets.schedule_retry/2,
          &DeliveryTargets.expire_target/2,
          &DeliveryTargets.invalidate_target/2
        ] do
    ...
    assert Repo.get!(DeliveryTarget, accepted.id).status == :provider_accepted
    assert Repo.aggregate(DeliveryTargetAttempt, :count, :id) == 1
  end
  ...
  test "rejected retry of an accepted target cannot reach the adapter through sync dispatch" do
    ...
    assert {:ok, %{status: :succeeded}} =
             DeliveryTargets.recompute_delivery(delivery, delivery.tenant_id)
    ...
    assert [] =
             Repo.all(from(a in DeliveryTargetAttempt, where: a.delivery_target_id == ^target.id))
  end
