...
| **B3: Synchronous workflow progression hook (worker call stack)** | 34-01 | 1 | FLOW-02 | — | After `Oban.drain_queue(:chimeway_delivery)` (no signals drain yet): `WorkflowRun.state` already advanced from `:waiting → :active` (progress path) OR set to terminal state (stop path) via `record_attempt/2 → progress_run/2` | E2E controller | `cd examples/chimeway_demo_host && mix test test/demo_host_web/controllers/feedback_pipeline_e2e_test.exs` | ❌ W0 | ⬜ pending |
