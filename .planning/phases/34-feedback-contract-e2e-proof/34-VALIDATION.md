...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| **B1: HTTP entry → Ingress write** | 34-01 | 1 | FLOW-01 | — | Endpoint returns 2xx; one `Chimeway.Webhooks.Ingress` row written with `normalized_status` populated and `tenant_id` set | E2E controller | `cd examples/chimeway_demo_host && mix test test/demo_host_web/controllers/feedback_pipeline_e2e_test.exs` | ✅ | ✅ green |
| **B2: Worker drain → DeliveryAttempt + canonical signal emit** | 34-01 | 1 | FLOW-01 | — | After `Oban.drain_queue(:chimeway_delivery)`: one `DeliveryAttempt` with canonical outcome atom (`:succeeded` or `:bounced`); one `Signal` row with `event_name == "chimeway.delivery.succeeded"` (or `.bounced`) | E2E controller | `cd examples/chimeway_demo_host && mix test test/demo_host_web/controllers/feedback_pipeline_e2e_test.exs` | ✅ | ✅ green |
...
| **B7: Trace fixture vocabulary alignment** | 34-02 | 1 | FLOW-01 | — | After fixture edit at `test/chimeway/traces_test.exs:416,523`: `mix test test/chimeway/traces_test.exs` green; `grep -n "chimeway.delivery.delivered" test/chimeway/traces_test.exs` returns 0 lines | unit (existing test rerun) | `mix test test/chimeway/traces_test.exs` | ✅ | ✅ green |
| **B8: Audit-closure artifact (FLOW-01/FLOW-02 mapping)** | 34-03 | 2 | FLOW-01, FLOW-02 | — | `34-VERIFICATION.md` exists with requirements table mapping FLOW-01 and FLOW-02 to evidence cells citing Phase 31 emission code, Phase 32 trace projection code, and the new Phase 34 E2E test (33-VERIFICATION.md:115-118 format); plan SUMMARY frontmatter declares `requirements-completed` covering FLOW-01 and FLOW-02 across the phase | manual file-shape check | `grep -E "^\| FLOW-0[12]" .planning/phases/34-feedback-contract-e2e-proof/34-VERIFICATION.md` | ✅ | ✅ green |
...
- [x] `examples/chimeway_demo_host/test/demo_host_web/controllers/feedback_pipeline_e2e_test.exs` — E2E file with progress + stop scenarios (B1–B6) — **created in commit `9a61387`** (34-01)
- [x] `test/chimeway/traces_test.exs:416,523` — fixture edits replacing `chimeway.delivery.delivered` with `chimeway.delivery.succeeded` (B7) — **commits `d918024`, `c5a1335`** (34-02)
- [x] `.planning/phases/34-feedback-contract-e2e-proof/34-VERIFICATION.md` — FLOW-01/FLOW-02 requirements table + Audit Notes section (B8) — **commit `77904bf`** (34-03)
- [x] Existing `test_helper.exs` and sandbox/Oban.Testing setup at `examples/chimeway_demo_host/test/` — reused unchanged from Phase 33
...
- [x] Wave 0 covers all MISSING references (B1–B6 closed by 34-01 E2E; B7 by 34-02 fixture fix; B8 by 34-03 verification artifact)
- [x] No watch-mode flags
- [x] Feedback latency ≤90s (root traces 0.7s + host E2E 0.3s ≪ budget)
- [x] `nyquist_compliant: true` set in frontmatter

**Approval:** approved (audit 2026-05-02)
...
| Metric | Count |
|--------|-------|
| Boundaries audited | 8 (B1–B8) |
| Gaps found | 0 |
| Resolved | 0 |
| Escalated | 0 |
| Tests run green | `mix test test/chimeway/traces_test.exs` (45 tests) + `mix test test/demo_host_web/controllers/feedback_pipeline_e2e_test.exs` (2 tests) |

**Result:** Phase 34 is Nyquist-compliant. All eight validation boundaries are covered by automated tests that run green. The two manual-only items (Audit Notes prose; `requirements-completed` frontmatter) both pass file-shape checks: `## Audit Notes` section present at `34-VERIFICATION.md:62`; phase-level closure of FLOW-01 and FLOW-02 declared by 34-01 and 34-03 SUMMARY frontmatter (34-02 declares only FLOW-01, which is correct — its scope was vocabulary drift only, contributing solely to FLOW-01 closure).
