...
## Phase 89 outcome (recorded 2026-07-30)

Phase 89 (Test-Lane Concurrency) converted **20 of 21** pure-DB `Chimeway.DataCase` modules to `async: true` (CONC-01), gave `Chimeway.Repo` an explicit test `pool_size` (CONC-02), brought `ci.test` to `--warnings-as-errors` parity (CONC-03), and proved no ordering coupling across 3 consecutive CI runs + a local `--seed 0` (CONC-04). All correctness criteria met; **1229 tests, 0 failures** on the 4-core runner across multiple random seeds (runs [30502499062](https://github.com/szTheory/chimeway/actions/runs/30502499062), [30502918529](https://github.com/szTheory/chimeway/actions/runs/30502918529)).

**Timing finding (honest):** the `test` lane wall-clock did **not** materially improve (~5–7 min/OTP leg, vs the ~321 s baseline). Measured locally, the 20 flipped modules are fast pure-DB unit tests contributing only ~4 s of a ~130 s suite; the wall-clock is dominated by heavier **serial** integration/mutator tests that must stay `async: false`. So the async conversion is the right correctness/hygiene step and sizes the pool for future concurrency, but **the <3 min target is not reached by async alone** — it remains gated by test execution the flip can't parallelize. Next lever: **Phase 90 pipeline tiering** (relocate the heavy full-matrix/browser lanes to a nightly tier so the PR path shrinks). The tracer also surfaced and fixed a real pool-sizing footgun: an uncapped `schedulers*2+10` pool exhausted the shared 100-connection Postgres (5 repos + throwaway prefix DBs) on high-core dev machines — capped at 24, leaving CI's value unchanged at 18.
