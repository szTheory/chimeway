...
**Goal (as set):** Make CI fast, deterministic, and trustworthy — collapse the ~6.5 min `main` wall-clock (gated by a single cold-compiling job) toward under ~3 min by fixing compiled-artifact caching, without lowering the quality signal or overcomplicating the pipeline. *(Delivered: correctness, observability, concurrency, tiering, supply-chain, reliability. Not delivered: the sub-3-min wall-clock itself — deferred.)*
...
- ✓ v1.18 Phase 102 Alpha Digital Twin & Hermetic Gate — TWIN-01/02 and GATE-01: a sanitized immutable-package host now exercises real Chimeway persistence, per-installation APNs dispatch, recovery, explanations, and pinned CrossWake protected-open decisions against deterministic host-owned seams. The runner scans actual stored facts, trace projections, telemetry, caught failures, redacted transport observations, and the exact candidate proof bytes before emission. Verified 3/3 roadmap truths with two byte-identical `mix verify.alpha_twin` runs, a green physical-proof contract, clean code review, and `mix ci.verify_gates` at 630 tests / 0 failures; zero human UAT.
...
- ✓ v1.16 Phase 91 Quality & Supply-Chain Polish — QUAL-01/02/03/04/05: `.tool-versions` (`erlang 27.3.4.15` / `elixir 1.19.5-otp-27`) is the single toolchain source, feeding all 14 `setup-beam` lanes via `version-file:` + `version-type: strict` and the cache keys; `.github/dependabot.yml` covers `mix` + `github-actions` (grouped weekly minor/patch); `ci.yml` runs least-privilege GITHUB_TOKEN (top-level `contents: read`, only obs-summary lanes escalate to `actions: read`, zero `: write` scopes); `mix ci.audit` runs both `hex.audit` + `deps.audit` (mix_audit `~> 2.1`, dev/test, `runtime: false`) advisory-only via `continue-on-error`; the 1.17 floor is wired into `ci-gate` `needs` and gates on push+nightly (`run_floor` == ci-gate's `if:`), skipped on PR. **Verified 5/5 by AUTOMATED live-CI assertion** (push 30556372077, nightly-dispatch 30556417111, PR #10 30556437942 — all green on fe3f732): all 14 lanes resolved identical `1.19.5`/`27.3.4.15`, matrix legs diverged (OTP 26 → `26.2.5.21`), floor → `1.17.3`/OTP 27; 0 checkout failures / 26 job-instances + `/jobs` timing query renders; both audit scanners printed hackney/decimal advisories with Lint green; floor+ci-gate success on push, skipped on PR with pr-gate green. Security: 7 threats closed, `threats_open: 0` (91-SECURITY.md).
...
- ✓ v1.16 Phase 89 Test-Lane Concurrency — CONC-01/02/03/04: the ~20 pure-DB `Chimeway.DataCase` modules flipped to `async: true` (the ~25 app-env + ~5 `:prefix` mutators correctly stay `async: false`); `config/test.exs` sets an explicit `Chimeway.Repo` `pool_size` sized for the async concurrency (the tracer caught a 5-repo Postgres pool-exhaustion hazard); `ci.test` runs `--warnings-as-errors` at parity with the `verify.*` lanes; **no ordering coupling** — CONC-04 proven by 3 consecutive green CI runs on fresh random seeds (30502499062/30502918529/30503332684) plus a local ordered `--seed 0` run, all 1229 tests / 0 failures. `lib/` untouched. **Honest finding:** the flip did not materially cut wall-clock (the 20 fast pure-DB modules are ~4s of a ~130s suite; wall-clock stays dominated by the serial integration/mutator tests that correctly remain serial) — the concurrency was a correctness/parity win, not the milestone's speed lever.
- ✓ v1.16 Phase 88 Cache Correctness & Compile-Once — CACHE-01/02/03/04 (**CACHE-05 deferred → backlog #4**): fixed the `MIX_ENV`-absent write-once cache-key collision so `_build` warms; standardized keys on env + resolved OTP/Elixir + root `mix.lock`; split source `deps` from compiled `build-test`; de-fragmented plain-hex lanes onto one shared warm key (partner accrue/threadline/sigra lanes keep graph-specific keys); added a producer `build` job with restore-only consumers (`fail-on-cache-miss: true`) and an explicit attributed `mix compile --warnings-as-errors` before `ecto.create`. Contract suites green (129 tests, 14/4 gate set intact); caches proven **HIT** on warm runs (`CACHE_MAIN_HIT`/`CACHE_DEPS_HIT` true) on producer + every consumer. **Accepted-risk:** CACHE-05 (near-zero recompile + sub-3-min warm `ci-gate`) NOT met — warm `ci-gate` regressed ~373s→~648s (serial producer ahead of consumers that still recompile ex_cldr ~86s + rebar deps + ~93 app files); owner banked correctness and deferred compile-once to a spike (2026-07-29).
...
**v1.19 Phase 104 CrossWake Provider-Feedback Recipe Truth complete (2026-09-12):** The adopter worker now uses the real public redaction and registry APIs and is tested as the exact documented snippet. Chimeway selects that documentation revision separately, replays it from a clean detached checkout, and keeps the v1.18 physical-proof branch fixed. DOCS-02/GATE-02 passed 6/6 verifier truths with no human UAT.

**v1.18 Phase 103 Physical iPhone & Adoption Truth complete (2026-09-12):** The bounded iPhone-first production path is now `physical_support_promoted` from one completion-bound, four-file, no-replace evidence snapshot. Machine proof independently records Chimeway delivery/provider-handoff/explanation facts and CrossWake permission/registration/protected-activation facts; the visible-alert observation remains a separate human attestation. TWIN-03 and DOCS-01 are complete without retaining credentials, raw provider responses, device or account identity, payloads, endpoints, logs, or media. APNs acceptance remains handoff only, and Android/FCM, generic background sync, device management, rich actions, analytics, and broad device support remain outside the claim.

**v1.18 Phase 102 Alpha Digital Twin & Hermetic Gate complete (2026-08-26):** The Alpha reference host is now reproducible from one SHA-validated Chimeway archive and exact detached CrossWake commit, with deterministic PostgreSQL lifecycle, host token custody, scripted APNs outcomes, crash recovery, protected-open reauthorization, and a closed ordered safety ledger. Runtime proof emission is gated by recursive scans of actual storage, traces, telemetry, caught failures, redacted provider observations, and final proof bytes. TWIN-01, TWIN-02, and GATE-01 passed 3/3 verifier truths; two hermetic runs were byte-identical, the code review was clean, and the final release gate passed 630 tests. Phase 103 Physical iPhone & Adoption Truth is next; active validation and security capabilities remain pre-advance follow-ups.
