...
- [x] **WRK-02**: Each workflow step can define explicit progression rules based on elapsed time or the prior delivery outcome.
...
- [x] **ESC-01**: Teams can escalate from one step to the next when a configured wait expires or a prior delivery outcome requires follow-up.
- [x] **ESC-02**: Teams can stop or cancel remaining workflow steps when a configured terminal condition is met.
- [x] **ESC-03**: Workflow progression and escalation remain idempotent and concurrency-safe under retries, duplicate claims, or repeated host calls.

## API & Integration

- [x] **API-01**: Host applications can submit explicit workflow progression signals through a stable public API without mutating durable history directly.
...
- [x] **INT-03**: Documentation and examples show how to model a common SaaS journey such as `in_app -> email escalation` with both sync and Oban-backed operation.

## Operator Explainability

- [x] **OPS-03**: Operators can inspect the current workflow position, completed steps, pending next action, and the reason a workflow advanced, waited, escalated, or stopped.
- [x] **OPS-04**: Journey traces preserve payload-safe explanation across multiple deliveries and channels under one workflow run.
...
| WRK-02 | 25 | completed | Time/outcome-based progression rules |
| WRK-03 | 24 | completed | Completed by plan 24-02 workflow run and transition persistence |
| ESC-01 | 26 | completed | Escalation between workflow steps |
| ESC-02 | 26 | completed | Stop/cancel terminal semantics |
| ESC-03 | 25 | completed | Idempotent progression and race safety |
| API-01 | 27 | completed | Stable host signal API |
| API-02 | 24 | completed | Completed by plan 24-01 replay-safe workflow declaration storage |
| INT-03 | 28 | completed | Docs and reference flows |
| OPS-03 | 27 | completed | Journey inspection surface |
| OPS-04 | 27 | completed | Payload-safe chain-level traces |