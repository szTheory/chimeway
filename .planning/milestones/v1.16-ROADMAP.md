...
- [x] **Phase 88: Cache Correctness & Compile-Once** — Fixed the MIX_ENV-absent cache-key collision, standardized keys, split deps from `_build`, de-fragmented plain-hex lanes, and added a producer job (CACHE-01..04). CACHE-05 (compile-once / sub-3min) deferred → backlog #4 (completed 2026-07-29, accepted-risk)
- [x] **Phase 89: Test-Lane Concurrency** — Flip ~32 pure-DB DataCase files to async, size the pool, enforce warnings-as-errors, and shake out ordering coupling across seeds (completed 2026-07-29)
...
- [x] 88-01-PLAN.md — Tracer: `build` producer job + `lint`/`install_golden` converted to restore-only consumers + CACHE-03 warnings-as-errors (wave 1)
- [x] 88-02-PLAN.md — Fan-out: 7 default-graph lanes → shared cache consumers; docs/OTP-matrix/partners reschemed to distinct self-cached roles; contract extended (wave 2)
- [x] 88-03-PLAN.md — Proof: delta-ledger recorded honestly (cache correctness proven; compile-once win NOT met → CACHE-05 deferred to backlog #4 per owner decision) (wave 3)
...
| 88. Cache Correctness & Compile-Once | 3/3 | Complete (accepted-risk: CACHE-05 deferred → backlog #4) | 2026-07-29 |
| 89. Test-Lane Concurrency | 6/6 | Complete | 2026-07-29 |
