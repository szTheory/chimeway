...
- **(b) Harden the always-running harness tests** so an absent integration turns **RED**, not silently green/skipped. Mirror the *principle* across all three lanes, but the *exact symbol differs per integration direction* (RESEARCH Coordinate 3 + Pitfalls 1–2 — do not mirror the literal `trigger/3` check):
  - **Sigra (IN):** `test/chimeway/integrations/sigra_auth_harness_test.exs` is single-guarded on `Code.ensure_loaded?(Sigra)` and today only checks `Sigra.Auth` → goes GREEN while the integration is missing (the vacuous footgun). Add a hard-assert that `Sigra.Integrations.Chimeway` loads AND exports its **verified** seam function. NOTE: `trigger/3` is on `Chimeway`, NOT on the integration module — the live lifecycle test calls `Sigra.Integrations.Chimeway.dispatch_magic_link_after_request/3`. The planner MUST grep the pinned `deps/sigra` for the actual public export and assert a function verified to exist (do not assume `trigger/3`).
  - **Accrue (IN):** `accrue_dunning_harness_test.exs` is *double*-guarded (`Code.ensure_loaded?(Accrue) and Code.ensure_loaded?(Accrue.Integrations.Chimeway)`) so it silently SKIPS when absent. Move the integration-module check OUT of the outer guard so absence goes RED, and assert `function_exported?(Accrue.Integrations.Chimeway, :start_campaign, 3)` (its real verified seam fn).
  - **Threadline (OUT):** there is NO `Threadline.Integrations.Chimeway` — chimeway owns `Chimeway.Telemetry.ThreadlineReporter` in `lib/`. Assert the OUT-direction reporter loads (`Code.ensure_loaded?(Chimeway.Telemetry.ThreadlineReporter)` + its export, e.g. `attach/0`) plus the partner fn `Threadline.record_action/2`. Mirroring a `*.trigger/3` check here would fail to compile.
- **(c) Floor-assert test counts:** add to `test/chimeway/release_gate_contract_test.exs` a floor assertion that each verify lane runs ≥ its expected integration-test count (doc-contract discipline applied to test execution). Verified current counts (use `≥`, not `==`, to catch regression-to-0 without brittleness): sigra root lane ≥ 5, sigra demo-host proof ≥ 2 (separate mix project), accrue ≥ 11, threadline ≥ 7. NOTE `verify.sigra` is TWO `mix test` invocations (root `--only sigra` + demo-host) — floor each separately. Closes WARNING-2.
