# CI Hardening Backlog — CI-only lane failures (3 of 3 resolved; #2/#3 VERIFIED-FIXED on CI in Phase 92/REL-02)
...
After the hygiene sweep (clean tree, 340 commits pushed, all systemic CI issues fixed), `ci-gate` was at 11/15 lanes green. **The flaky `test` lane (#1) was fixed 2026-07-09.** **Phase 92/REL-02 (2026-07-30) closed the last two: #2 (`demo.up --check`) and #3 (Accrue path-dep) are VERIFIED-FIXED on a phase-HEAD push run — `ci-gate` is now fully green** (push [`30573877353`](https://github.com/szTheory/chimeway/actions/runs/30573877353) @ `eff0ba43`, all lanes `success`). All three original CI-only failures are resolved; tracking issue [#4](https://github.com/szTheory/chimeway/issues/4) is closed.
...
- **✅ VERIFIED-FIXED (Phase 92/REL-02):** proven on this phase's own HEAD push run [`30573877353`](https://github.com/szTheory/chimeway/actions/runs/30573877353) @ `eff0ba43` — `verify_example` (Example host + admin smoke) **and** `verify_journeys` (TeamPulse consumer journeys) both `success`. The pinned mechanism above (job-level `DATABASE_URL` inherited by the `System.cmd` subprocess + the `:dev` pre-warm compile step) is confirmed load-bearing and must not be removed (Pitfall 5). The interim `@tag timeout: 300_000` is **retained** — tightening to `120_000` is an optional future cleanup, not part of REL-02 (which is the verified-fixed decision itself), and would need its own guarded push-proof pass to avoid landing an unguarded too-tight timeout.
...
- **✅ VERIFIED-FIXED (Phase 92/REL-02):** proven on this phase's own HEAD push run [`30573877353`](https://github.com/szTheory/chimeway/actions/runs/30573877353) @ `eff0ba43` — `verify_accrue` (Accrue dunning integration gate) `success`. The pinned mechanism above (full-tree `mix deps.compile` via `obs-recompile.sh` producing `accrue.app` in dependency order + the nested `ACCRUE_PATH` layout) is confirmed as the real fix.
