...
**Escalation shape:** Email 1 delivers immediately → workflow enters `:waiting` on the `wait_until` step for up to 48h → Email 2 fires only if no Outcome Signal arrives.
...
- **Verification:** `ACCRUE_PATH=../accrue/accrue mix verify.accrue --warnings-as-errors` (root lifecycle + demo host proof)
